export const db = (() => {
  const DB_NAME = 'budget-db'; const VER = 1; let _db;
  function open(){ return new Promise((res,rej)=>{ if(_db) return res(_db); const req = indexedDB.open(DB_NAME, VER);
    req.onupgradeneeded = e => { const d = e.target.result;
      ...
      if (!d.objectStoreNames.contains('transactions')) { const os = d.createObjectStore('transactions', { keyPath: 'id' }); os.createIndex('byDate','date'); os.createIndex('byCategory','categoryId'); }
    ...
    }; req.onsuccess = ()=> res(_db = req.result); req.onerror = ()=> rej(req.error); }); }
  async function tx(names, mode='readonly'){ return (await open()).transaction(names, mode); }
